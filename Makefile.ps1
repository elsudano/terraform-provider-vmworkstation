...
Function publish { ## This option prepare the zip files to publishing in Terraform Registry
    ...
    install 
    compress
    [string]${HASH} = Get-FileHash -Path ${ZIPFILE} | Select-Object Hash
    ${HASH} | Out-File -FilePath ${SHAFILE}
    Write-Host "we did Publish and the Hash is: ${HASH}"
}
...
Function prepare_environment {
    ...
    [string]${BINARY} = "${PREFIX}-${NAME}_v${VERSION}"
    [string]${ZIPFILE} = "${PREFIX}-${NAME}_${VERSION}_${OS}_${ARCH}.zip"
    [string]${SHAFILE} = "${PREFIX}-${NAME}_${VERSION}_SHA256SUMS"
